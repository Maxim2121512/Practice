...
public class CustomIntegerParser {
	...
	public static int parseInteger(String string, Integer maxNumber) throws InputNumberException, OutOfFieldException {
		try {
			int number = Integer.parseInt(string);
			if (maxNumber == null) {
				throw new InputNumberException("First initialize field size");
			}
			if(number < 0) {
				throw new InputNumberException("Caught a negative number", string);
			} if (number >= maxNumber) {
